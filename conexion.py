...
def crear_usuario(correo,contra):
    if existe_usuario(correo):
        ...
        cursor.execute(insertar, (correo,h))#contra cammbiar por h para no ver contraseña
        bd.commit()
...
def insertar_pelicula(pelicula):
    ...
    insertar = "INSERT INTO pelicula \
        (titulo,fecha_visto,imagen,director,anyo,usuarioId) \
        VALUES (%s,%s,%s,%s,%s,%s)"
    cursor.execute(insertar,(titulo,fecha_visto,imagen,director,anyo,usuarioId))
    bd.commit()
...
def get_peliculas():
    query="SELECT id,titulo,imagen,fecha_visto,director,anyo FROM pelicula"
